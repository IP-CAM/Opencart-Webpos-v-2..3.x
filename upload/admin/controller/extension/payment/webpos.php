...
class ControllerExtensionPaymentWebPos extends Controller {
	...
	public function index() {
		...
		$data['tab_add_url'] = $this->url->link('extension/payment/webposbuilder', 'token=' . $this->session->data['token'], 'SSL');
		...
		'href' => $this->url->link('extension/payment/webpos', 'token=' . $this->session->data['token'], 'SSL')
		);

		$data['action'] = HTTPS_SERVER . 'index.php?route=extension/payment/webpos&token=' . $this->session->data['token'];
		...
		$banks=$this->model_extension_payment_webposbuilder->getbanks(array('sort'=>'bank_id'));
		//fix key sort order
		$new_banks=array();
		...
		$this->response->setOutput($this->load->view('extension/payment/webpos.tpl', $data));
	}


	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/payment/webpos')) {
	...
	public function install() {
		if (!$this->user->hasPermission('modify', 'extension/payment/webpos')) {
			...
			$this->load->model('extension/payment/webpos');

			$this->model_extension_payment_webpos->install();
	...
	public function uninstall() {
		if (!$this->user->hasPermission('modify', 'extension/payment/webpos')) {
			...
			$this->model_extension_payment_webpos->uninstall();
