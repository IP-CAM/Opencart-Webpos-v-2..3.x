<?php 
class ControllerExtensionPaymentWebposBuilder extends Controller {
	private $error = array();
 
	public function index() {
		$this->load->language('extension/payment/webposbuilder');

		$this->document->setTitle($this->language->get('heading_title'));
		
		$this->load->model('extension/payment/webposbuilder');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_extension_payment_webposbuilder->addbank($this->request->post);
			...
			$this->response->redirect($this->url->link('extension/payment/webposbuilder', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_extension_payment_webposbuilder->editbank($this->request->get['bank_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $bank_id) {
				$this->model_extension_payment_webposbuilder->deletebank($bank_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/payment/webposbuilder', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		
		$data['add'] = $this->url->link('extension/webposbuilder/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('extension/payment/webposbuilder/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$bank_total = $this->model_extension_payment_webposbuilder->getTotalbanks();
		
		$results = $this->model_extension_payment_webposbuilder->getbanks($filter_data);
		...
		foreach ($results as $result) {
			...
			$data['banks'][] = array(
				...
				'edit'      => $this->url->link('extension/payment/webposbuilder/edit', 'token=' . $this->session->data['token'] . '&bank_id=' . $result['bank_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_name'] = $this->url->link('extension/payment/webposbuilder', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('extension/payment/webposbuilder', 'token=' . $this->session->data['token'] . '&sort=status' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('extension/payment/webposbuilder', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('extension/payment/webposbuilder_list.tpl', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['bank_id'])) { 
			$data['action'] = $this->url->link('extension/payment/webposbuilder/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('extension/payment/webposbuilder/edit', 'token=' . $this->session->data['token'] . '&bank_id=' . $this->request->get['bank_id'] . $url, 'SSL');
		}
		
		$data['cancel'] = $this->url->link('extension/payment/webposbuilder', 'token=' . $this->session->data['token'] . $url, 'SSL');
		
		if (isset($this->request->get['bank_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$bank_info = $this->model_extension_payment_webposbuilder->getbank($this->request->get['bank_id']);
		}
		...
		$this->response->setOutput($this->load->view('extension/payment/webposbuilder_form.tpl', $data));
	}

	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'extension/payment/webposbuilder')) {
	...
	$this->load->model('extension/payment/webposbuilder');
	$result = $this->model_extension_payment_webposbuilder->checkShortName($short);
	return $result;
	}
